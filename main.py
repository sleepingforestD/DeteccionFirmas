...
async def predict(file: UploadFile = File(...)):
    ...
    try:
        ...
        # Salida del modelo: [[p_real]]
        prob_real = float(prediction[0][0])      # probabilidad de clase 1 (real)
        prob_falsa = 1.0 - prob_real             # probabilidad de clase 0 (falsa)

        # Decisión con umbral 0.5
        threshold = 0.5
        predicted_class = 1 if prob_real >= threshold else 0

        class_names = {0: "falsa", 1: "real"}
        class_name = class_names[predicted_class]

        confidence = prob_real if predicted_class == 1 else prob_falsa
